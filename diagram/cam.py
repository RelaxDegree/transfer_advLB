...
import numpy as np
import cv2
from torchvision.models import MobileNet_V2_Weights, ResNet50_Weights, DenseNet121_Weights
...
for i in range(0, 5):
    print('{:.3f} -> {}'.format(probs[i], classes[idx[i] + 1]))
# generate class activation mapping for the top1 prediction
# 输出与图片尺寸一致的CAM图片
CAMs = returnCAM(features_blobs[0], weight_softmax, [idx[0]])
# render the CAM and output
print('output CAM.jpg for the top1 prediction: %s' % classes[idx[0] + 1])
...
cv2.imwrite(root + '--mode' + str(model_id) + 'CAM.jpg', result)
